...
from src.settings import ExtractSettings, SecretSettings
...
def save_to_json(data, type, save_dir=ExtractSettings.JSON_PATH_CMC):
    ...
    filename = os.path.join(save_dir, f"{type}_map.json")
    try:
        with open(filename, 'w') as f:
            json.dump(data, f, indent=4)
    except Exception as e:
        print(f"Erreur pendant la sauvegarde des données : {e}")
...
def fetch_map(url):
    ...
    try:
        ...
        print(f"Erreur durant la récupération des données : {e}")
        return None
