...
@with_session
def get_all_trading_pairs(session: Session) -> List[TradingPair]:
    """Récupère toutes les paires de trading de la base de données"""
    return session.query(TradingPair).all()


@with_session
def get_trading_pair_by_id(trading_pair_id: int, session: Session) -> Optional[TradingPair]:
    """Récupère une paire de trading par son ID"""
    return session.query(TradingPair).filter(TradingPair.id == trading_pair_id).first()


@with_session
def get_trading_pair_by_currencies(base_currency_id: int, quote_currency_id: int, session: Session) -> Optional[TradingPair]:
...
@with_session
def get_trading_pairs_by_base_currency(base_currency_id: int, session: Session) -> List[TradingPair]:
...
@with_session
def get_trading_pairs_by_quote_currency(quote_currency_id: int, session: Session) -> List[TradingPair]:
...
@with_session
def search_trading_pairs(search_term: str, session: Session) -> List[TradingPair]:
